...
const char input_file_name[]="IMDB-MULTI_10.json";
...
void read_dataset(const char* s) {
	int arr[n_graphs];
	rand_distribution(arr, n_graphs, n_graphs);
...
struct Model {
	...
	double get_exp(autoGrad_state& qs, int graph_idx, double* temp_grad = nullptr)const {
		...
		for (int i = 0; i < 1 << n_qBits; i++) {
			if (i % n_graph_label != graph_label[graph_idx])
				continue;
...
int main(int argn, char** args) {
	...
	const int train_begin = 0, train_end = 600;
	const int valid_begin = 600, valid_end = 700;
	const int test_begin = 700, test_end = 800;
